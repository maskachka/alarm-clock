...
AlarmListScreen::AlarmListScreen(AlarmListScreenListener& l) : listener_(l), root_(nullptr), rows_{} {}
void AlarmListScreen::build(lv_obj_t* parent) {
  ...
  constexpr Ui::ButtonSize kAlarmRowIconButtonSize{32, 32};
  ...
  for (uint8_t i = 0; i < AlarmService::kMaxAlarms; ++i) {
    ...
    Ui::createButton(group, UiIcon::kEdit, UiTheme::kSecondaryButtonStyle, kAlarmRowIconButtonSize, onSet, &r);
    Ui::createButton(group, UiIcon::kDelete, UiTheme::kSecondaryButtonStyle, kAlarmRowIconButtonSize, onDelete, &r);
    Ui::createButton(group, UiIcon::kAlarm, UiTheme::kSecondaryButtonStyle, kAlarmRowIconButtonSize, onRingtone, &r);
    r.enabled_switch = lv_switch_create(r.root);
    lv_obj_set_size(r.enabled_switch, 48, 32);
...
void AlarmListScreen::onRingtone(lv_event_t* e) {
  auto* r = static_cast<Row*>(lv_event_get_user_data(e));
  if (r) r->screen->listener_.onAlarmRingtoneSettingsRequested(r->index);
}
